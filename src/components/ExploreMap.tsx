...
const redPointerIcon = L.icon({
  iconUrl: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'%3E%3Cpath d='M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z' fill='%23d93025'/%3E%3Ccircle cx='12' cy='9' r='2.5' fill='white'/%3E%3C/svg%3E",
  iconSize: [28, 28],
  iconAnchor: [14, 28],
  popupAnchor: [0, -28],
});
...
function BoundsController({ markers }: { markers: MarkerPlace[] }) {
  const map = useMap();
...
export function ExploreMap({ items }: ExploreMapProps) {
  const markers = useMemo<MarkerPlace[]>(
    () =>
      items.map((item) => {
